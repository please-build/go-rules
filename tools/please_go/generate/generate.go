...
func New(srcRoot, thirdPartyFolder, hostModFile, module, version, subrepo string, buildFileNames, moduleDeps, install []string, buildTags []string, extraLabels []string) *Generate {
	moduleArg := module
	if version != "" {
		moduleArg += "@" + version
	}
...
func (g *Generate) rule(rule *Rule) *bazelbuild.Rule {
	...
	r.SetAttr("labels", NewStringList(g.extraLabels))
